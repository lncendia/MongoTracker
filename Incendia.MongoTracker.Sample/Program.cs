...
using Incendia.MongoTracker.Sample.Contexts;
...
async Task InitializeDatabaseAsync(MongoDbContext mongoDbContext, ModelBuilder configuration)
{
  ...
  await mongoAuthorTracker.SaveChangesAsync(mongoDbContext.Authors);
  ...
  await mongoBookTracker.SaveChangesAsync(context.Books);
}
...
ModelBuilder Configure()
{
  ...
  builder.Entity<Audiobook>(e =>
  {
    e.Property(b => b.Author).IsConcurrencyToken();
  });
